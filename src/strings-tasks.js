...
function isPalindrome(str) {
  const str1 = str
    ...
    .join('')
    .toLowerCase();
  for (let i = 0; i <= Math.floor(str1.length / 2); i += 1) {
...
function reverseWords(str) {
  ...
  return array.map((word) => reverseString(word)).join(' ');
}
...
function encodeToRot13(str) {
  const inputAlp = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const outputAlp = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    const index = inputAlp.indexOf(str[i]);
    res += outputAlp[index] || str[i];
  }
  return res;
}
